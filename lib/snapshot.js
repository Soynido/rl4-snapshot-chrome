...
class RL4SnapshotGenerator {
  ...
  _buildUltraSnapshot(input) {
    ...
    const prunedDecisions = decisions
      ...
      .map((d) => ({
        id: d.id,
        intent: d.intent,
        choice: this._excerpt(d.chosen_option || ''),
        rationale: this._excerpt(d.intent_text || '')
      }));
    ...
    const ultra = {
      ...
      timestamp: nowIso,
      context_state: digest.context_state || {},
      topics: prunedTopics,
      decisions: prunedDecisions,
      timeline_macro,
      ...(semanticHints
        ? this._ultraSemanticHints({ digest, prunedTopics, prunedDecisions, timeline_macro, messages: msgs })
        : {}),
      conversation_fingerprint: {
        algorithm: 'sha256',
  ...
  _ultraSemanticHints(input) {
    ...
    // Extract checklist items from decision "choice" text (often contains explicit "Si ..." clauses).
    const checklist = [];
    ...
    for (const c of candidates) {
      // FR style: "Si ..."
      if (!/^si\s+/i.test(c)) continue;
      const item = c.replace(/\s+/g, ' ').trim();
      if (item.length < 8) continue;
      checklist.push(item.length > 140 ? item.slice(0, 137) + '...' : item);
