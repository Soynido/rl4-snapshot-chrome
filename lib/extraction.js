...
function extractTopics(messages) {
  ...
  return top.map((t) => {
    const message_refs = [];
    const recentFirst = [...messages].reverse(); // Plus récents d'abord
    ...
    for (const m of recentFirst) {
      ...
      if (cleaned.includes(t.w)) {
        if (message_refs.length < 3) {
          ...
          break; // Stop dès qu'on a 3
        }
      }
    }

    message_refs.reverse(); // Remettre ordre chronologique
...
function extractDecisions(messages) {
  ...
  for (const m of messages) {
    ...
    if (/recommend/i.test(text)) {
      ...
      intent = 'recommend';
      intentText = 'Recommendation (FR)';
    } else if (/on\s+devrait|il\s+faut/i.test(text)) {
      intent = 'propose';
      intentText = 'Proposition (FR)';
    } else if (/décision\s*:/i.test(text)) {
      intent = 'decide';
      intentText = 'Décision explicite (FR)';
    }
    ...
    if (explicit && explicit[1]) chosen = sanitizeChoice(explicit[1]);
      ...
      const rec = text.match(/\b(recommend|choose|recommande|choisis|choisir)\b\s+(.+)/i);
